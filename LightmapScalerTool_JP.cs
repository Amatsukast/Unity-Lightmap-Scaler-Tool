...
public class LightmapScalerTool_JP : EditorWindow
{
    ...
    [MenuItem("Tools/ライトマップスケール調整ツール")]
    public static void ShowWindow()
    {
        var window = GetWindow<LightmapScalerTool_JP>("ライトマップスケール調整ツール");
    ...
    private void AdjustScalesSurfaceArea()
    {
        ...
        // 有効なオブジェクトが選択されていない場合
        if (totalOcc < 0f || all == null || all.Count == 0)
        {
            // MeshRendererが無い場合もここで拾える
            statusMessage = "選択したオブジェクトが有効なオブジェクトではありません。";
            return;
        }
