...
public class LightmapScalerTool : EditorWindow
{
    ...
    public static void ShowWindow()
    {
        ...
        window.minSize = new Vector2(300, 460); // 初期サイズ（最小サイズ）
    }
    ...
    private void OnGUI()
    {
        ...
        // --- 追加：一括設定UI ---
        GUILayout.Label("選択オブジェクト配下の scale in lightmap を一括設定");

        // フィルター入力欄
        includeNameFilter = EditorGUILayout.TextField("対象名フィルター（含む）", includeNameFilter);
        excludeNameFilter = EditorGUILayout.TextField("除外名フィルター（含む）", excludeNameFilter);
